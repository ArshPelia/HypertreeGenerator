...
def get_firstxsimpletrees(node, x = 1):
    trees = []
    for child in node.childNodes:
        if child.nodeType == child.ELEMENT_NODE:
            trees.append(getSimpleTree(child))
            if len(trees) == x:
                return trees
    return trees

def get_simpleTreesRecursive(node):
    trees = []
    for child in node.childNodes:
        if child.nodeType == child.ELEMENT_NODE:
            trees.append(getSimpleTree(child))
            # trees.extend(get_simpleTreesRecursive(child))
    return trees
...
def main():
    ...
    print("")
    print(Fore.RED +"All simple Trees from root:")
    print(Style.RESET_ALL)
    trees = get_simpleTreesRecursive(root)
    for tree in trees:
        tree.show()

    print("")
    print(Fore.GREEN +"Simple Tree from body:")
    print(Style.RESET_ALL)
    trees = getSimpleTree(table)
    trees.show()
